...
8. While watching for events, Witnesses observe an EventTypeLock and then begin to construct a prophecy claim.
9. Witnesses will then sign the prophecyID of the event with their EVM native keys and then send that signature along with the prophecy to the oracle module using `SetGlobalNonceProphecyID()`. See [Virtual Mem Pool](Concepts#virtualMem) for more details.
...
15. BridgeBank will then instruct the ERC20 bridgeToken to mint funds for the user in the amount specified by the prophecy.

## Fees
Anytime a user wants to export cosmos assets onto an EVM chain, there are three types of fees that need to be accounted for, user incurred fees,  relayer incurred fees, and witness incurred fees.
...
## How to Initiate a Cosmos-Side lock
