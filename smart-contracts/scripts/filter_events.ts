...
async function fetchTransfers(cosmosAddress: string) {
  pgClient.query({
    text: "",
    values: [],
  });
...
// Lock + Burn separated
// TODO: Return type?
async function fetchEVMExports(cosmosAddress: string) {
  pgClient.query({
    text: "",
    values: [],
  });
...
async function fetchIBCExports(cosmosAddress: string) {}

function getEthereumEvents(AttackerCosmosAddress: string): void {}

function getAllEthereumEvents(): void {}

async function main() {
  // Get EVM Smart contract events
  ...
  const BurnEvents = await bridgeBank.queryFilter(
    bridgeBank.filters.LogBurn(),
    EthereumStartingHeight
  );
  const LockEvents = await bridgeBank.queryFilter(
    bridgeBank.filters.LogLock(),
    EthereumStartingHeight
  );
  const UnlockEvents = await bridgeBank.queryFilter(
    bridgeBank.filters.LogUnlock(),
    EthereumStartingHeight
  );
  const MintEvents = await bridgeBank.queryFilter(
    bridgeBank.filters.LogBridgeTokenMint(),
    EthereumStartingHeight
  );

  async function ParseLockBurn(
    BurnOrLock: typeof BurnEvents[0] | typeof LockEvents[0]
  ): Promise<BurnOrLockEvent> {
    const LockOrBurn = {
      ...
      FormattedValue: ParseValue(
        BurnOrLock.args?._symbol,
        BurnOrLock.args?._value
      ),
  ...
  async function ParseUnlock(
    Unlock: typeof UnlockEvents[0]
  ): Promise<UnlockOrMintEvent> {
    const UnlockEvent = {
  ...
  // Note: Hmm is this typeof useful? args are null coalesced
  async function ParseMint(
    Mint: typeof MintEvents[0]
  ): Promise<UnlockOrMintEvent> {
    const MintEvent = {
  ...
  const Burns = await Promise.all(
    BurnEvents.map(async (burn) => await ParseLockBurn(burn))
  );
  const Locks = await Promise.all(
    LockEvents.map(async (lock) => await ParseLockBurn(lock))
  );
  const Unlocks = await Promise.all(
    UnlockEvents.map(async (unlock) => await ParseUnlock(unlock))
  );
  const Mints = await Promise.all(
    MintEvents.map(async (mint) => await ParseMint(mint))
  );
  const Exports = await cosmosBridge.queryFilter(
    cosmosBridge.filters.LogProphecyCompleted(),
    EthereumStartingHeight
  );
  ...
  for (const AttackerCosmosAddress of AttackerCosmosAddresses) {
    AttackerBurns.push(
      ...Burns.filter((burn) => burn.CosmosAddress === AttackerCosmosAddress)
    );
    AttackerLocks.push(
      ...Locks.filter((lock) => lock.CosmosAddress === AttackerCosmosAddress)
    );
  }

  const AttackerEthereumAddresses = Array.from(
    new Set<string>([
      ...AttackerBurns.map((burn) => burn.EthereumAddress),
      ...AttackerLocks.map((lock) => lock.EthereumAddress),
    ])
  );
  for (const ethereumAddress of AttackerEthereumAddresses) {
    AttackerUnlocks.push(
      ...Unlocks.filter((unlock) => unlock.EthereumAddress === ethereumAddress)
    );
    AttackerMints.push(
      ...Mints.filter((mint) => mint.EthereumAddress === ethereumAddress)
    );
  }
...
main()
  ...
  .catch((error) => console.error("Error Reported: ", error));
