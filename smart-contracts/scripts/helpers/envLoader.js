module.exports.loadEnv = function () {
  ...
  if (process.env.OPERATOR.length === 0) {
    return console.error("Must provide operator address as environment variable.");
  }
  ...
  if (process.env.PAUSER.length === 0) {
    return console.error("Must provide pauser address as environment variable.");
  }
  ...
  if (initialPowers.length !== initialValidators.length) {
    return console.error("Each initial validator must have a corresponding power specified.");
  }
  initialPowers = initialPowers.map((e) => {
    return parseInt(e);
  });
