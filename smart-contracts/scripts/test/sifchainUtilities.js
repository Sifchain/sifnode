...
const sharedYargOptions = {
  ...
  ethereum_private_key_env_var: {
    describe:
      "an environment variable that holds a single private key for the sender\nnot used for localnet",
    demandOption: false,
...
function processArgs(context, args = {}) {
  ...
  const result = yargs(process.argv.slice(4)).options(args).strict().argv;
  return result;
}
