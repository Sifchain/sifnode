...
async function main() {
  ...
  if (useForking) await impersonateBridgeBankAccounts(container, hardhat);
  ...
  for (const addr of whitelistData.array) {
    if (await bridgeBankAsOperator.getTokenInEthWhiteList(addr.address)) {
      ...
      console.log(
        `\x1b[31mToken ${addr.address} NOT added to the whitelist: already there, no transaction sent\x1b[0m`
      );
      continue;
    }
  ...
  if (addressList.length > 0) {
    ...
    const encodedData = factory.interface.encodeFunctionData("bulkWhitelistUpdateLimits", [
      addressList,
    ]);
    ...
    // UX
    console.log(
      `\x1b[46m\x1b[30mSending transaction. This may take a while, please wait...\x1b[0m`
    );
    ...
    // logs in red
    console.log(
      `\x1b[31mFailed to whitelist tokens: the final token list is empty. Were all tokens already whitelisted?\x1b[0m`
    );
  }
...
async function estimateGasPrice() {
  ...
  console.log(
    `Using ideal Gas price: ${hardhat.ethers.utils.formatUnits(finalGasPrice, "gwei")} GWEI`
  );
