...
async function main() {
  ...
  const { consensusThreshold, operator, initialValidators, initialPowers, owner, pauser } =
    getEnv();
  ...
  const bank = await upgrades.deployProxy(BridgeBank, [cosmosBridge.address, owner, pauser]);
