...
export class SifnodedAdapter {
  ...
  async executeSifBurn(
    ...
    let sifnodedCmd: string = `${this.gobin}/sifnoded tx ethbridge burn ${sender.account} ${destination.address} ${amount} ${symbol} ${crossChainFee} --network-descriptor ${networkDescriptor} --keyring-backend test --gas-prices=0.5rowan --gas-adjustment=1.5 --chain-id localnet --home ${this.homedir} --from ${sender.name} --output json -y `
