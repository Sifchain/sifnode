...
interface EthereumAddressAndKey {
    privateKey: string
    address: string
}

function isWallet(s: SignerWithAddress | Wallet): s is Wallet {
    return 'privateKey' in s;
}

// should get this from hardhat, but it's nontrivial, so I just copied it
const HARDHAT_NETWORK_MNEMONIC =
    "test test test test test test test test test test test junk";

function signerWithAddressToEthereumAddress(s: SignerWithAddress | Wallet): EthereumAddressAndKey | null {
    if (isWallet(s)) {
        return {
            address: s.address,
...
function signerArrayToEthereumAccounts(accounts: SignerWithAddress[], nValidators: number): EthereumAccounts {
    const xs = accounts.map(signerWithAddressToEthereumAddress)
