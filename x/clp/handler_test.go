...
func CalculateWithdraw(t *testing.T, keeper clpkeeper.Keeper, ctx sdk.Context, asset clptypes.Asset, signer string, wBasisPoints string, asymmetry sdk.Int) sdk.Coins {
	...
	if asymmetry.IsPositive() {
		swapResult, _, _, _, err := FEATURE_TOGGLE_MARGIN_CLI_ALPHA_SwapOne(ctx, app.ClpKeeper, clptypes.GetSettlementAsset(), swapAmount, asset, pool, sdk.OneDec(), sdk.NewDecWithPrec(3, 3))
		assert.NoError(t, err)
	...
	if asymmetry.IsNegative() {
		swapResult, _, _, _, err := FEATURE_TOGGLE_MARGIN_CLI_ALPHA_SwapOne(ctx, app.ClpKeeper, asset, swapAmount, clptypes.GetSettlementAsset(), pool, sdk.OneDec(), sdk.NewDecWithPrec(3, 3))
		assert.NoError(t, err)
...
func CalculateSwapReceived(t *testing.T, keeper clpkeeper.Keeper, tokenRegistryKeeper tokenregistrytypes.Keeper, ctx sdk.Context, assetSent clptypes.Asset, assetReceived clptypes.Asset, swapAmount sdk.Uint) sdk.Uint {
	...
	assert.NoError(t, err)
	emitAmount, _, _, _, err := FEATURE_TOGGLE_MARGIN_CLI_ALPHA_SwapOne(ctx, keeper, assetSent, swapAmount, clptypes.GetSettlementAsset(), inPool, sdk.OneDec(), sdk.NewDecWithPrec(3, 3))
	assert.NoError(t, err)
	_, err = tokenRegistryKeeper.GetEntry(registry, outPool.ExternalAsset.Symbol)
	assert.NoError(t, err)
	emitAmount2, _, _, _, err := FEATURE_TOGGLE_MARGIN_CLI_ALPHA_SwapOne(ctx, keeper, clptypes.GetSettlementAsset(), emitAmount, assetReceived, outPool, sdk.OneDec(), sdk.NewDecWithPrec(3, 3))
	assert.NoError(t, err)
	return emitAmount2
}
