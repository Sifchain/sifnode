...
func CalculateWithdraw(t *testing.T, keeper clpkeeper.Keeper, ctx sdk.Context, asset clptypes.Asset, signer string, wBasisPoints string, asymmetry sdk.Int) sdk.Coins {
	...
	if asymmetry.IsPositive() {
		swapResult, _, _, _, err := clpkeeper.SwapOne(clptypes.GetSettlementAsset(), clpkeeper.NewMustNat(&swapAmount), asset, pool, sdk.OneDec())
		assert.NoError(t, err)
	...
	if asymmetry.IsNegative() {
		swapResult, _, _, _, err := clpkeeper.SwapOne(asset, clpkeeper.NewMustNat(&swapAmount), clptypes.GetSettlementAsset(), pool, sdk.OneDec())
		assert.NoError(t, err)
...
func CalculateSwapReceived(t *testing.T, keeper clpkeeper.Keeper, tokenRegistryKeeper tokenregistrytypes.Keeper, ctx sdk.Context, assetSent clptypes.Asset, assetReceived clptypes.Asset, swapAmount sdk.Uint) sdk.Uint {
	...
	assert.NoError(t, err)
	emitAmount, _, _, _, err := clpkeeper.SwapOne(assetSent, clpkeeper.NewMustNat(&swapAmount), clptypes.GetSettlementAsset(), inPool, sdk.OneDec())
	assert.NoError(t, err)
	_, err = tokenRegistryKeeper.GetEntry(registry, outPool.ExternalAsset.Symbol)
	assert.NoError(t, err)
	emitAmount2, _, _, _, err := clpkeeper.SwapOne(clptypes.GetSettlementAsset(), clpkeeper.NewMustNat(&emitAmount), assetReceived, outPool, sdk.OneDec())
	assert.NoError(t, err)
	return emitAmount2
}
