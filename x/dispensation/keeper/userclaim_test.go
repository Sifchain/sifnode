...
func TestKeeper_GetClaimsByType(t *testing.T) {
	...
	fetchList := keeper.GetClaimsByType(ctx, types.DistributionType_DISTRIBUTION_TYPE_VALIDATOR_SUBSIDY).UserClaims
	assert.Len(t, fetchList, numberOfClaims)
	fetchList = keeper.GetClaims(ctx).UserClaims
...
func TestKeeper_LockClaim(t *testing.T) {
	...
	lockedList := keeper.GetClaims(ctx).UserClaims
