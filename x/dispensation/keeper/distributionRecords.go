...
func (k Keeper) SetDistributionRecord(ctx sdk.Context, dr types.DistributionRecord) error {
	...
	key := types.GetDistributionRecordKey(dr.DistributionStatus, dr.DistributionName, dr.RecipientAddress.String())
	store.Set(key, k.cdc.MustMarshalBinaryBare(dr))
	return nil
}

func (k Keeper) GetDistributionRecord(ctx sdk.Context, distributionName string, recipientAddress string, status types.DistributionStatus) (types.DistributionRecord, error) {
...
func (k Keeper) ExistsDistributionRecord(ctx sdk.Context, distributionName string, recipientAddress string, status types.DistributionStatus) bool {
	key := types.GetDistributionRecordKey(status, distributionName, recipientAddress)
	if k.Exists(ctx, key) {
		return true
	}
