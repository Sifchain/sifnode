...
func TestNewHandler_CreateDistribution_PayRewardsInAnyToken_HappyCase(t *testing.T) {
	...
	money := keeper.GetBankKeeper().GetBalance(ctx, sdk.AccAddress(outputList[0].Address), "ceth")
	money1 := keeper.GetBankKeeper().GetBalance(ctx, sdk.AccAddress(outputList[0].Address), "catk")
	money2 := keeper.GetBankKeeper().GetBalance(ctx, sdk.AccAddress(outputList[0].Address), "rowan")
	money3 := app.BankKeeper.GetAccountsBalances(ctx)
	lpAddess, err := sdk.AccAddressFromBech32("sif1acdh3rca2elta9jdg5a6mjsw2cv3map6d8uc0x")
