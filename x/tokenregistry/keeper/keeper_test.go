...
func TestKeeper_CheckDenomPermissions(t *testing.T) {
	...
	app.TokenRegistryKeeper.SetToken(ctx, &types.RegistryEntry{
		Denom:       "t2",
		Decimals:    18,
		Permissions: []types.Permission{types.Permission_IBCEXPORT, types.Permission_IBCIMPORT},
	})
	...
	assert.True(t, app.TokenRegistryKeeper.CheckDenomPermissions(ctx, "t2", []types.Permission{types.Permission_IBCEXPORT}))
