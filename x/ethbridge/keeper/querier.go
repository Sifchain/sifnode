...
func NewLegacyQuerier(keeper Keeper, cdc *codec.LegacyAmino) sdk.Querier { //nolint
	...
	return func(ctx sdk.Context, path []string, req abci.RequestQuery) ([]byte, error) {
		...
		case types.QueryPauser:
			return legacyQueryPauser(ctx, cdc, req, keeper)
		default:
...
func legacyQueryPauser(ctx sdk.Context, cdc *codec.LegacyAmino, query abci.RequestQuery, keeper Keeper) ([]byte, error) { //nolint
	var req types.QueryPauserRequest
	if err := cdc.UnmarshalJSON(query.Data, &req); err != nil {
		return nil, sdkerrors.Wrap(types.ErrJSONMarshalling, fmt.Sprintf("failed to parse req: %s", err.Error()))
	}
	queryServer := NewQueryServer(keeper)
	response, err := queryServer.GetPauserStatus(sdk.WrapSDKContext(ctx), &req)
	if err != nil {
		return nil, err
	}
	return cdc.MarshalJSONIndent(response, "", "  ")
