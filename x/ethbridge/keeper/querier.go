...
func NewLegacyQuerier(keeper Keeper, cdc *codec.LegacyAmino) sdk.Querier {
	...
	return func(ctx sdk.Context, path []string, req abci.RequestQuery) ([]byte, error) {
		...
		case types.QueryCrosschainFeeConfig:
			return legacyQueryCrosschainFeeConfig(ctx, cdc, req, keeper)
		default:
...
func legacyQueryCrosschainFeeConfig(ctx sdk.Context, cdc *codec.LegacyAmino, query abci.RequestQuery, keeper Keeper) ([]byte, error) {
	var req types.QueryCrosschainFeeConfigRequest
	...
	response, err := queryServer.CrosschainFeeConfig(sdk.WrapSDKContext(ctx), &req)
	if err != nil {
		return nil, err
	}
