...
func getProphecyHandler(cliCtx client.Context, storeName string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		networkIDString := vars[restNetworkID]
		networkID, err := strconv.ParseInt(networkIDString, 10, 64)
		if err != nil {
		...
		bz, err := cliCtx.LegacyAmino.MarshalJSON(
			types.NewQueryEthProphecyRequest(
				oracletypes.NetworkID(networkID), bridgeContract, nonceString, symbol, tokenContract, ethereumSender))
		if err != nil {
