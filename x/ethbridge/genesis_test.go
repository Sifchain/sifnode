...
func TestExportGenesis(t *testing.T) {
	...
	assert.Equal(t, receiver, state.NativeTokenReceiveAccount)
}
...
func TestInitGenesis(t *testing.T) {
	...
	assert.Equal(t, len(state.PeggyTokens), tokenscount)
	assert.Equal(t, state.NativeTokenReceiveAccount, receiver)
	state2 := ethbridge.ExportGenesis(ctx2, keeper2)
	assert.Equal(t, len(state2.PeggyTokens), 0)
	assert.Equal(t, state2.NativeTokenReceiveAccount, "")
	...
	actualReceiver := keeper2.GetNativeTokenReceiverAccount(ctx2)
...
func CreateState(ctx sdk.Context, keeper ethbridge.Keeper, t *testing.T) (int, string) {
	...
	keeper.SetNativeTokenReceiverAccount(ctx, receiver)
	set := keeper.IsNativeTokenReceiverAccount(ctx, receiver)
	assert.True(t, set)
