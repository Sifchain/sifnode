...
func TestShouldConvertIncomingCoins(t *testing.T) {
	...
	entry1 := app.TokenRegistryKeeper.GetEntry(registry, "ueth")
	require.NotNil(t, entry1)
	entry1c := app.TokenRegistryKeeper.GetEntry(registry, entry1.UnitDenom)
	...
	entry2 := app.TokenRegistryKeeper.GetEntry(registry, "cusdt")
	require.NotNil(t, entry2)
	entry2c := app.TokenRegistryKeeper.GetEntry(registry, entry2.UnitDenom)
...
func TestGetConvForIncomingCoins(t *testing.T) {
	...
	entry1 := app.TokenRegistryKeeper.GetEntry(registry, "ueth")
	require.NotNil(t, entry1)
	entry2 := app.TokenRegistryKeeper.GetEntry(registry, "ceth")
	require.NotNil(t, entry2)
	entry1c := app.TokenRegistryKeeper.GetEntry(registry, entry1.UnitDenom)
	require.NotNil(t, entry1c)
	diff := uint64(entry1c.Decimals - entry1.Decimals)
...
func TestIsRecvPacketAllowed(t *testing.T) {
	...
	registry := app.TokenRegistryKeeper.GetRegistry(ctx)
	entry1 := app.TokenRegistryKeeper.GetEntry(registry, "ibc/44F0BAC50DDD0C83DAC9CEFCCC770C12F700C0D1F024ED27B8A3EE9DD949BAD3")
	require.NotNil(t, entry1)
	...
	entry2 := app.TokenRegistryKeeper.GetEntry(registry, "ibc/A916425D9C00464330F8B333711C4A51AA8CF0141392E7E250371EC6D4285BF2")
	require.NotNil(t, entry2)
	...
	entry3 := app.TokenRegistryKeeper.GetEntry(registry, "rowan")
...
func TestExecConvForIncomingCoins(t *testing.T) {
	...
	registry := app.TokenRegistryKeeper.GetRegistry(ctx)
	mintedDenomEntry := app.TokenRegistryKeeper.GetEntry(registry, mintedDenom)
	...
	convertToDenomEntry := app.TokenRegistryKeeper.GetEntry(registry, mintedDenomEntry.UnitDenom)
