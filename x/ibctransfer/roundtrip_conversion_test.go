...
func TestExportImportConversionEquality(t *testing.T) {
	...
	rEntry := app.TokenRegistryKeeper.GetEntry(registry, "rowan")
	require.NotNil(t, rEntry)
	mrEntry := app.TokenRegistryKeeper.GetEntry(registry, "microrowan")
	...
	mrEntryUnit := app.TokenRegistryKeeper.GetEntry(registry, mrEntry.UnitDenom)
