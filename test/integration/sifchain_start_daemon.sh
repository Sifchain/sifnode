...
whitelisted_validator=$(yes $VALIDATOR1_PASSWORD | sifnoded keys show -a --bech val $MONIKER --home $CHAINDIR/.sifnodecli)
...
adminuser=$(yes | sifnoded keys add sifnodeadmin --keyring-backend test -o json 2>&1 | jq -r .address)
