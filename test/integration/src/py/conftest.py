...
def sifnode_base_dir():
    return test_utilities.get_required_env_var("SIFNODE_BASE_DIR")
...
def bridgebank_address():
    return env_or_truffle_artifact("BridgeBank", "BRIDGE_BANK_ADDRESS", smart_contract_artifact_dir, ethereum_network_id)
...
def bridgetoken_address(smart_contract_artifact_dir, ethereum_network_id):
    return env_or_truffle_artifact("BridgeToken", "BRIDGE_TOKEN_ADDRESS", smart_contract_artifact_dir, ethereum_network_id)
...
def ethereum_network_id(is_ropsten_testnet):
    ...
    if result:
        ...
        result = 1 if is_ropsten_testnet else 5777
        return result
