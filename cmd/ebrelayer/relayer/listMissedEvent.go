...
func NewListMissedCosmosEvent(networkDescriptor uint32, tmProvider, ethProvider string, registryContractAddress common.Address,
		...
		for _, result := range block.TxsResults {
			for _, event := range result.Events {
				...
				case types.MsgBurn, types.MsgLock:
					...
					cosmosMsg, networkDescriptor, err := txs.BurnLockEventToCosmosMsg(claimType, event.GetAttributes(), list.SugaredLogger)
					if err != nil {
						log.Println(err.Error())
						continue
					}

					if networkDescriptor == list.NetworkDescriptor && !MessageProcessed(cosmosMsg, ProphecyClaims) {
