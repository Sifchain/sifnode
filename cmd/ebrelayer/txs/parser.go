...
func BurnLockEventToCosmosMsg(claimType types.Event, attributes []tmKv.Pair, sugaredLogger *zap.SugaredLogger) (types.CosmosMsg, uint32, error) {
	...
	for _, attribute := range attributes {
		...
		case types.CosmosSenderSequence.String():
			...
			if !ok {
				...
				return types.CosmosMsg{}, networkDescriptor, errors.New("invalid account sequence: " + val)
			}
		...
		case types.EthereumReceiver.String():
			...
			if !common.IsHexAddress(val) {
				...
				return types.CosmosMsg{}, networkDescriptor, errors.New("invalid recipient address: " + val)
			}
		...
		case types.Symbol.String():
			...
			if claimType == types.MsgBurn {
				if !strings.Contains(val, defaultSifchainPrefix) {
					...
					return types.CosmosMsg{}, networkDescriptor, errors.New("can only relay burns of '%v' prefixed coins" + defaultSifchainPrefix)
				}
		...
		case types.Amount.String():
			...
			if !ok {
				...
				return types.CosmosMsg{}, networkDescriptor, errors.New("invalid amount:" + val)
			}
			amount = tempAmount
		case types.NetworkDescriptor.String():
			...
			if err != nil {
				sugaredLogger.Errorw("network descriptor can't parse", "networkDescriptor", val)
				return types.CosmosMsg{}, networkDescriptor, errors.New("can only relay burns of '%v' prefixed coins" + defaultSifchainPrefix)
			}
	...
	return types.NewCosmosMsg(claimType, cosmosSender, cosmosSenderSequence, ethereumReceiver, symbol, amount), networkDescriptor, nil
}
