...
func nodeCreateCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "create [chain-id] [moniker] [mnemonic] [admin-clp-addresses] [ip_addr] [peer-address] [genesis-url]",
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if len(args) == 5 {
				sifgen.NewSifgen(&args[0]).NodeCreate(args[1], args[2], adminCLPAddresses, args[4], nil, nil, &printDetails, &withCosmovisor)
			} else {
				sifgen.NewSifgen(&args[0]).NodeCreate(args[1], args[2], adminCLPAddresses, args[4], &args[5], &args[6], &printDetails, &withCosmovisor)
			}
		},
	}
}
