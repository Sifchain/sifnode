...
namespace :cluster do
  ...
  namespace :sifnode do
    namespace :deploy do
      ...
      task :standalone, [:chainnet, :provider, :namespace, :image, :image_tag, :moniker, :mnemonic, :admin_clp_addresses, :admin_oracle_addresses] do |t, args|
        check_args(args)
        ...
        cmd = %Q{helm upgrade sifnode #{cwd}/../../deploy/helm/sifnode \
          ...
          --set sifnode.args.adminOracleAddress=#{args[:admin_oracle_addresses]} \
          --install -n #{ns(args)} --create-namespace \
