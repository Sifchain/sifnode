<template>
  <div class="slippage-section mh-70">
    ...
    <div class="field-wrappers d-flex fd-row j-start">
      <div
        class="slippage-item"
        v-bind:class="{ 'slippage-item__selected': localSlippage === '0.5' }"
        @click="handleUpdateSlippage('0.5')"
      >
        <p class="text--standard text--normal text--regular">0.5%</p>
      </div>
      <div
        class="slippage-item"
        v-bind:class="{ 'slippage-item__selected': localSlippage === '1.0' }"
        @click="handleUpdateSlippage('1.0')"
      >
        <p class="text--standard text--normal text--regular">1.0%</p>
      </div>
      <div
        class="slippage-item"
        v-bind:class="{ 'slippage-item__selected': localSlippage === '2.0' }"
        @click="handleUpdateSlippage('2.0')"
      >
        <p class="text--standard text--normal text--regular">2.0%</p>
      </div>
    ...
    function validateSlippage() {
      try {
        if (!isNumeric(localSlippage.value)) {
          ...
          !(
            localSlippage.value.startsWith(".") &&
            localSlippage.value.length === 2
          )
        ) {
...
.custom-slippage-input-container {
  position: relative;
  margin: 0 16px;
}
