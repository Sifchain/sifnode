...
const assetAmounts = [USDC, USDT].map(tok => AssetAmount.create(tok, JSBI.BigInt(100)));
describe('queryListOfAvailableTokens', () => {
    describe('updateListOfAvailableTokens', () => {
        ...
        it('should deliver tokens in order', () => {
            expect(store.state.availableAssetAccounts.map(({ asset: { symbol }, amount }) => ({ symbol, amount }))).toEqual([
                {
                    symbol: 'USDC',
...
//# sourceMappingURL=queryListOfAvailableTokens.test.js.map